'use strict';
/* ═══════════════════════════════════════════════════════════════
   우노 온라인 · uno-rt.js  (Firebase RTDB · 호스트 권한)
   - 마스터가 엔진을 들고 모든 수를 검증·적용
   - 각 플레이어는 자기 손패만 수신, 자기 차례에 수 전송(play/draw)
   - firebase-config.js 비어있으면 비활성 → 패스앤플레이만
   - rooms/{CODE} 사용(원카드·도둑잡기와 동일 보안규칙), game:'uno' 로 구분
   ═══════════════════════════════════════════════════════════════ */
(function () {
  ...
  const toast = (m, ms) => (window.UApp ? UApp.toast(m, ms) : (window.Fx && Fx.toast(m, ms)));
  ...
  RT.initUI = function () {
    ...
    if (!status) return;
    if (typeof firebase === 'undefined') { status.textContent = '오프라인 · Firebase 미로드 — 패스앤플레이만 가능'; return; }
    if (!cfgValid()) { status.textContent = '오프라인 · firebase-config.js 필요 — 패스앤플레이만 가능'; return; }
    try {
    ...
    } catch (e) { status.textContent = '온라인 초기화 실패: ' + (e.message || e); console.error('[우노]', e); }
  ...
  RT.host = function (cfg) {
    if (!ready) { toast('온라인 비활성 — firebase-config.js 확인', 1200); return; }
    isHost = true; UApp.isHost = true; UApp.mode = 'online';
    roomCode = code4(); myPid = newPid(); UApp.myPid = myPid;
    const myName = (document.getElementById('rt-myname').value || '').trim() || (cfg.players[0] && cfg.players[0].name) || '마스터';
    const room = {
      game: 'uno', phase: 'lobby', host: myPid,
      config: { startCards: cfg.startCards, rules: cfg.rules },
      players: {}, move: null, state: null,
    };
    room.players[myPid] = { id: myPid, name: myName, joinedAt: Date.now() };
    toast('방 생성 중…', 800);
    ref().set(room).then(() => {
      UApp.showGame(); document.getElementById('room-info').innerHTML = `방 <b>${roomCode}</b>`;
      openLobby(); listenRoom(); listenMovesAsHost();
    }).catch(err => {
      console.error('[우노] 방 생성 실패', err);
      toast(/permission/i.test(err.message || '') ? '쓰기 권한 거부 — DB 규칙 확인' : '방 생성 실패: ' + err.message, 1600);
      isHost = false; UApp.mode = 'hotseat';
    });
  };

  RT.join = function (code, name) {
    if (!ready) { toast('온라인 비활성 — firebase-config.js 확인', 1200); return; }
    roomCode = code.toUpperCase(); myPid = newPid(); UApp.myPid = myPid; isHost = false; UApp.isHost = false; UApp.mode = 'online';
    ref().get().then(snap => {
      if (!snap.exists()) { toast('그런 방이 없어요 (코드 확인)', 1200); return; }
      const room = snap.val();
      if (room.game !== 'uno') { toast('이 방은 우노 방이 아니에요', 1300); return; }
      if (room.phase !== 'lobby') { toast('이미 시작된 방이에요', 1200); return; }
      return ref('players/' + myPid).set({ id: myPid, name, joinedAt: Date.now() }).then(() => {
        UApp.showGame(); document.getElementById('room-info').innerHTML = `방 <b>${roomCode}</b>`;
        openLobby(); listenRoom();
      });
    }).catch(err => {
      console.error('[우노] 참가 실패', err);
      toast(/permission/i.test(err.message || '') ? '읽기 권한 거부 — DB 규칙 확인' : '참가 실패: ' + err.message, 1600);
    });
  };
  ...
  function ensureLobby() {
    ...
    el.querySelector('#lobby-copy').onclick = () => { navigator.clipboard?.writeText(location.origin + location.pathname + '?room=' + roomCode); toast('초대 링크 복사됨', 900); };
    return el;
  }
  ...
  function renderLobby(room) {
    ...
    el.querySelector('#lobby-list').innerHTML = players.map(p => `<div class="seat" style="min-width:auto;padding:8px 14px">${esc(p.name)}${p.id === room.host ? ' <span class="uno-badge">마스터</span>' : ''}</div>`).join('');
  ...
  RT.startGame = function () {
    ...
    ref().get().then(snap => {
      const room = snap.val();
      const players = Object.values(room.players || {}).sort((a, b) => a.joinedAt - b.joinedAt).map(p => ({ id: p.id, name: p.name }));
      if (players.length < 2) return;
      game = new UnoGame({ players, startCards: room.config.startCards, rules: room.config.rules });
      publish('playing');
    });
  };
  ...
  function listenMovesAsHost() {
    ...
    const cb = r.on('value', snap => {
      const mv = snap.val(); if (!mv || !game || game.winner) return;
      if (game.players[game.turn].id !== mv.pid) { ref('move').set(null); return; } // 차례 검증
      if (mv.type === 'play') game.play(mv.pid, mv.cardId, mv.color ? { color: mv.color } : {});
      else if (mv.type === 'draw') game.drawAndPass(mv.pid);
  ...
  function listenRoom() {
    ...
    const st = stRef.on('value', s => { const pub = s.val(); if (pub) UApp.renderOnline(pub, myHand, myPid); });
    unsub.push(() => stRef.off('value', st));

    const hRef = ref('hands/' + myPid);
    const hc = hRef.on('value', s => { myHand = s.val() || []; ref('state').get().then(ss => { const pub = ss.val(); if (pub) UApp.renderOnline(pub, myHand, myPid); }); });
